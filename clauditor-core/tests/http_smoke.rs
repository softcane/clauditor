...
fn unique_db_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "clauditor-core-http-smoke-{}-{nanos}.db",
        std::process::id()
    ))
}

fn start_core(http_addr: &str, grpc_addr: &str, db_path: &PathBuf) -> CoreProcess {
    let child = Command::new(env!("CARGO_BIN_EXE_clauditor-core"))
        .env("CLAUDITOR_HTTP_ADDR", http_addr)
        .env("CLAUDITOR_GRPC_ADDR", grpc_addr)
        .env("CLAUDITOR_DB_PATH", db_path)
        .env("RUST_LOG", "error")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn clauditor-core");
...
fn wait_for_health(process: &mut CoreProcess, http_addr: &str) {
    ...
    while Instant::now() < deadline {
        if let Some(status) = process.try_wait() {
            panic!("clauditor-core exited before health check: {status}");
        }
    ...
    panic!("clauditor-core did not become healthy at {http_addr}");
}
...
fn open_db_after_schema_ready(db_path: &PathBuf) -> Connection {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            panic!("clauditor-core did not initialize sqlite schema");
        }
...
fn core_serves_health_and_metrics_on_configured_http_addr() {
    ...
    assert!(
        metrics.contains("clauditor_active_sessions"),
